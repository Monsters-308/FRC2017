...
public class Chassis extends PIDSubsystem {
	...
	// private final AdvancedRobotDrive robotDrive6 =
	// RobotMap.chassisRobotDrive6;
	...
	public void arcadeDrive(double forward, double turn) {
		...
		if (RobotConstants.enablePID == true) { // use PID process
			...
			if (turn == 0.0) {
				if (turning == true) { // Code is called first time, when we
										// stopped turning
					setPointTimer.start();
					RobotConstants.gyroPIDOutput = 0.0; // Reset PIDOutput to
														// zero
					turning = false; // Set turning to false, because we are not
										// turning any more
				} else if (setPointTimer.get() != 0) {// If this isn't the first
														// time
					if (setPointTimer.get() >= 1.0) { // Robot is moving
														// straight
						enablePID();
						gyro.reset();
	...
	public void setDrive(double left, double right) {
		...
		// SmartDashboard.putNumber("tra gyro error",
		// getPIDController().getError());
		// SmartDashboard.putNumber("tra left",left);
		// SmartDashboard.putNumber("tra right",right);
		// SmartDashboard.putNumber("tra robot
		// pid",RobotConstants.gyroPIDOutput);
		// tankDrive(-(left ),
		// (-(right )));
	...
	/**
	 * TODO: Remove unused function public int getEncoderPosition() { int
	 * intValue = (int) left2.getPosition(); return -intValue; // encodetemp =
	 * encodetemp +3; // return encodetemp; // return 0; }
	 **/
	 ...
	 * @author Alexander Kaschta
	 * @param leftValue
	 *            value for the left motors
	 * @param rightValue
	 *            value for the right motors
	 * @param squaredInputs
	 *            are the input values already squared?
	 */
	 ...
	 * @author Alexander Kaschta
	 * @param moveValue
	 *            value that show how much the robot should go forward (y-axis)
	 * @param rotateValue
	 *            value how much the robot should rotate (x-axis)
	 * @param squaredInputs
	 *            show's if the input values already had been squared (usually
	 *            true)
	 */
